...
def upload(username, passwd, filename):
    ...
    r = browser()
    content = re.match('(.+);(.+)', r.post('http://www.share-online.biz/upv3_session.php', {'username': username, 'password': passwd}).content)  # get upload_session and best server to upload
    ...
    content = r.post('http://%s' % (host), data, files={'fn': open(filename, 'rb')}).content  # upload
