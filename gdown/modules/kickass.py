...
def __login__(username, passwd):
    ...
    r = browser()
    data = {'return_uri': 'https://kickass.to/', 'email': username, 'password': passwd}
    rc = r.post('https://kickass.to/auth/socialize/', data).content
...
def rateGood(username, passwd, torrent_hash):
    ...
    if r is False:
        return False
    rc = JSONDecoder().decode(r.post('https://kickass.to/torrents/submitthnx/{}/'.format(torrent_hash)).content)
    print rc
    if rc['method'] == 'ok':
        return True
    else:
...
def accInfo(username, passwd):
    ...
    if r is False:
        ...
        rc = r.get('https://kickass.to/').content
        username = re.search('<a href="/user/(.+)/">profile</a>', rc).group(1)
        rc = r.get('https://kickass.to/user/{}'.format(username)).content
        open('gdown.log', 'w').write(rc)
...
def signUp(email, passwd, username=None):
    ...
    #rc = r.get('https://kickass.to/auth/register').content
    ...
    data = {'return_uri': 'https://kickass.to/user/{}'.format(username),
