...
def geturl(link, login, passwd):
	...
	return opera.get('http://api.wupload.com/link?method=getDownloadLink', values, log=False, stream=True)	# return connection
def status(login, passwd):
	opera = browser()
	values = { 'u':login, 'p':passwd }
	content = opera.get('http://api.wupload.com/user?method=getInfo', values)
